...
export const HomeProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchTickets = async () => {
    try {
      ...
      setResponseTicketsPending(pending);
      setResponseTicketsRejected(rejected);
      setResponseTicketsApproved(approved);
    } catch (error) {
  ...
  const handleChanges = ({
    ...
    changes: { [key: unknown]: string | number };
  }) => {
